...
var spacePhoto = document.getElementById('spacePhoto'); //These two functions are set up to clear the loading output after the picture has finished loading.
...
spaceButton.addEventListener('click', function () {
  ...
  console.log(date); //Setting the proper format that the API requires.

  var spaceDate = date.toISOString().substr(0, 10); //Preparing the API request, using our api key and our random date. It took me a little while to figure out API keys.

  var spaceURL = 'https://api.nasa.gov/planetary/apod?api_key=Aj3aZZ1ZX9H5YfwheMgVWGyYVwghvILvpOU9MGvt&date=' + spaceDate; //Setting our output clear on a timer.

  wait(clearOutput); //Getting the data from NASA's API

  fetch(spaceURL).then(function (response) {
    outputDiv.innerHTML = "Photo loading...";
    ...
    return spacePhoto.src = data.url;
  }).catch(function (error) {
    return console.log('There was an error:', error);
  });
}, false); // shipController.getShips().then((data) => {
//   console.log(data);
// });
